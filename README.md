...
[**ACPs（Agent Communication Protocol Suite）**](https://github.com/AIP-PUB) 智能体通信协议族的官方生态接入启动项目（Starter Project）。

**本项目旨在帮助任何基于 Skill 或 MCP 的智能体快速、无缝地接入 ACPs 生态系统。**

ACPs 由北京邮电大学牵头、中国电子技术标准化研究院参与制定，定义了智能体间发现（ADP）、交互（AIP）和可信注册（ATR）三组核心协议。作为接入 ACPs 生态的基础脚手架，本项目展示了如何让一个通用大模型智能体轻松化身为 ACPs 生态中的 Leader Agent：
- 在 **ATR** 流程下完成智能体登记与 mTLS 证书准备，建立可信身份；
- 通过 **ADP** 在生态内发现所需的 Partner Agent；
- 通过 **AIP** 委派任务并汇总结果，向最终用户提供强大的统一交互入口。
...
按后文完成各组件配置后，依次启动（首次运行会自动创建 venv 并安装依赖）：
...
提交前请确认 ACS 已正确配置 `x-caChallengeBaseUrl`（见上一步），须待 **审核通过** 后方可进行证书申请与对外互通。
